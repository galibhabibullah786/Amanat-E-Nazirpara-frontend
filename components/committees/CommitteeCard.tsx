...
export default function CommitteeCard({ committee, isCurrent, onMemberClick, isRight, index }: CommitteeCardProps) {
  const members = committee.memberDetails || [];
  
  const designationOrder = ['president', 'vice-president', 'secretary', 'treasurer', 'member'];
  const sortedMembers = [...members].sort((a: CommitteeMember, b: CommitteeMember) => 
    designationOrder.indexOf(a.designation) - designationOrder.indexOf(b.designation)
  );

  const president = sortedMembers.find((m: CommitteeMember) => m.designation === 'president');
  const vicePresident = sortedMembers.find((m: CommitteeMember) => m.designation === 'vice-president');
  const secretary = sortedMembers.find((m: CommitteeMember) => m.designation === 'secretary');
  const treasurer = sortedMembers.find((m: CommitteeMember) => m.designation === 'treasurer');
  const otherMembers = sortedMembers.filter((m: CommitteeMember) => m.designation === 'member');
            ...
            {otherMembers.length > 0 && (
              <div className="pt-4 border-t border-gray-100">
                ...
                <div className="flex justify-center items-start flex-wrap gap-3 sm:gap-4">
                  {otherMembers.map((member: CommitteeMember) => (
                    <div key={member.id} className="flex flex-col items-center w-[55px] sm:w-[65px]">
                      <MemberAvatar 
                        member={member} 
                        size="sm"
                        onSelect={onMemberClick}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
              {committee.members.map((member: string, idx: number) => (
                <motion.div
                  ...
                  className="flex items-center gap-3 p-2.5 rounded-xl bg-gray-50 hover:bg-primary-50 transition-colors group/item"
                >
                  <div className="w-9 h-9 rounded-full bg-gradient-to-br from-primary-500 to-primary-700 flex items-center justify-center text-white text-xs font-bold shadow-sm group-hover/item:scale-105 transition-transform">
